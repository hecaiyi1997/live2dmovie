...
local addNewChangedTaskInfo = function ( roleID, taskInfo )
	...
	--taskInfos.taskList = tb--一次完整对话表
	--taskInfos.taskList={taskInfos.taskList}
	--Task.taskInfos[tb.cur_role_id] = taskInfos

	local taskInfos = {}---20231005添加4句，xml更新后这里也要保持最新任务单表
	taskInfos.taskList = taskInfo--一次完整对话表
	taskInfos.taskList={taskInfos.taskList}
	Task.taskInfos[taskInfo.roleID] = taskInfos
	...
	if taskInfos and taskInfos.taskList then
		for i,v in pairs(taskInfos.taskList) do
			...
			if v.subType == TaskConst.SubType.Talk and subCfg and subCfg.contentID == npcID then
				...
				return taskIDs;--20231005 add，只取一个对话
			end
		end
	end
	return taskIDs
		...
		--if TaskConst.ClientDoTask[taskInfo.subType] then
			...
			if cfg and cfg.subTasks then
				...
				if isFinishTask then
					...
					local taskInfos = {}---20231005添加4句，xml更新后这里也要保持最新任务单表
					taskInfos.taskList = nextTaskInfo--一次完整对话表
					taskInfos.taskList={taskInfos.taskList}
					Task.taskInfos[nextTaskInfo.roleID] = taskInfos
					addNewChangedTaskInfo(userInfo.cur_role_id, nextTaskInfo)
				else
				--skynet.timeout(1,function()
