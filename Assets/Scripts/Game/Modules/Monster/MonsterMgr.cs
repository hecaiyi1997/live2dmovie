...
public class MonsterMgr
{
	...
	{
        Debug.Log("AddMonster typeID/uid: "+typeID+":"+uid);
        ...
        monsterGameOE.transform.SetParent(container);
            ...
            monsterGameOE.gameObject.AddComponent<Rigidbody2D>();
            monsterGameOE.gameObject.AddComponent<BoxCollider2D>();
            monsterGameOE.gameObject.layer = LayerMask.NameToLayer("Monster");
            Debug.Log("AddMonster pos : " + pos.x+" "+pos.y+" "+pos.z);
    ...
    private void InitMonster(GameObjectEntity monsterGameOE, long uid, long typeID, Vector3 pos, Vector3 targetPos, float curHp, float maxHp)
    {
        ...
        dynamicBuffer.Add(new DamageEvent { instigator = monster, damage = 0, direction = Vector3.zero, impulse = 0 });
            ...
            Timer.Register(2.0f, () => {
                rmq.UpdateNavAgent();
                monsterGameOE.gameObject.GetComponent<monsterapi>().origpos = monsterGameOE.gameObject.transform.position;
            });
        }
    ...
    private void CreateLooks(Entity ownerEntity, long typeID)
    {
        ...
        Debug.Log("CreateLooks CreateLooks===" + resID);
        if (ResMgr.GetInstance().HasLoadedPrefab(resID))
        {
            Debug.Log("HasLoadedPrefab CreateLooks===" + resID);
        ...
        else
        {
            ...
            Debug.Log("CreateLooks===" + bodyPath);
