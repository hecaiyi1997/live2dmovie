...
public class RoleMgr
{
	...
	{
        Vector3 ps = new Vector3 { x = -0.8f, y = 10, z = 0 };
        ...
        roleGameOE.gameObject.AddComponent<Rigidbody2D>();
        roleGameOE.gameObject.AddComponent<BoxCollider2D>();
        roleGameOE.gameObject.layer = LayerMask.NameToLayer("Player");
        ...
        cam.Follow = roleGameOE.transform;
        cam.LookAt = roleGameOE.transform;

        Timer.Register(2.0f, () => {
                RoleMgr.GetInstance().UpdateMainRoleNavAgent();
            //SkillManager.GetInstance().lookat(roleGameOE.gameObject.transform, roleGameOE.gameObject.transform);
            //SkillManager.GetInstance().CastSkill(110000); 
        });
            Timer.Register(4.0f, () => {
                XLuaManager.Instance.onMoveStart.Invoke();
    ...
    private void CreateLooks(Entity ownerEntity)
    {
            ...
            XLuaFramework.ResourceManager.GetInstance().LoadAsset<GameObject>(bodyPath, delegate (UnityEngine.Object[] objs) {
                if (objs != null && objs.Length > 0)
                {
                    ...
                    //var parentTrans = ownerEntity.transform;
                    Debug.Log("SpawnRoleLooks parentTrans : " + parentTrans.name);
                    Debug.Log("SpawnRoleLooks bodyOE.transform : " + bodyOE);
                    //parentTrans.localPosition = new Vector3 { x = 965, y = 550, z = 1005 } ;
                    bodyOE.transform.SetParent(parentTrans);
                    bodyOE.transform.localPosition = new Vector3(0,0,0);
