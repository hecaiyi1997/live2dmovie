...
public class SkillManager
{
    ...
    public long GetSkillCD(int skillID)
    {
        ...
        foreach(var item in skillCDEndTime){
            Debug.Log("skillCDEndTime==="+item.Key+item.Value);
        }
        return endTime;
    }
    ...
    private static int GetAttackID(int career, int comboIndex)
    {
        ...
        //次movie项目中caree=1，所以timeline资源名字为skill_11000,个位数自己加
   ...
   public void setRatio(float r)
   {
            GameObject vcam = GameObject.Find("CM vcam1");
            CinemachineVirtualCamera cam = vcam.GetComponent<CinemachineVirtualCamera>();
            cam.m_Lens.OrthographicSize = r;// 1.1f;
   }
