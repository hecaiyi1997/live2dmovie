...
public class MovementUpdateSystem : BaseComponentSystem
{
    ...
    protected override void OnUpdate()
    {
        ...
        for (int i=0; i<targetPositions.Length; i++)
        {
            ...
            //Debug.Log("isAutoFinding=" + isAutoFinding);
            if (isAutoFinding)
            {
            ...
            else
            {
                ...
                if (moveDistance < speed*dt)
                {
                    //目标已经离得很近了
                    newPos = targetPos;
                    //Debug.Log("--MovementUpdateSystem moveDistance < speed*dt targetPos=" + targetPos.x + ":" + targetPos.y + ":" + targetPos.z);
                    //Debug.Log("--MovementUpdateSystem moveDistance < speed*dt newPos=" + newPos.x + ":" + newPos.y + ":" + newPos.z);
                }
                else
                {
                    newPos = startPos+groundDir*speed*dt;
                   // Debug.Log("--MovementUpdateSystem targetPos=" + targetPos.x + ":" + targetPos.y + ":" + targetPos.z);
                    //Debug.Log("--MovementUpdateSystem newPos=" + newPos.x + ":" + newPos.y + ":" + newPos.z);
                }
                newPos.y = startPos.y;
                ...
                Debug.Log("xx--MovementUpdateSystem newPos=" + newPos.x + ":" + newPos.y + ":" + newPos.z);
        ...
        if (curLocoStateObj.LocoState!=LocomotionState.State.BeHit && curLocoStateObj.LocoState!=LocomotionState.State.Dead)
        {
            ...
            if (input.sqrMagnitude > 0)
            {
                ...
                */
                ...
                float3 targetDirection = new Vector3(input.x, input.y,0);
                float3 curPos = posArray[0].localPosition;
                var speed = moveSpeedArray[0].CurSpeed;
                var newTargetPos = new TargetPosition();
                Debug.Log("XXXXXYYYYY" + targetDirection.x + ":" + targetDirection.y + ":"+targetDirection.z+":speed:"+speed);
                var delt = curPos + targetDirection * (speed * 0.010f);
                var dirtime= targetDirection * (speed * 0.010f);
                delt.x = curPos.x + dirtime.x;
                delt.y = curPos.y + dirtime.y;
                delt.z = curPos.z + dirtime.z;
                Debug.Log("dirtimeXXXXXYYYYY==" + dirtime.x + "/" + dirtime.y + "/" + dirtime.z);
                Debug.Log("curposXXXXXYYYYY==" + curPos.x + "/" + curPos.y + "/" + curPos.z);
                Debug.Log("dta+curposXXXXXYYYYY" + delt.x + ":" + delt.y + ":" + delt.z);
                if (speed > 0)
                    newTargetPos.Value = curPos + targetDirection * (speed * 0.010f);//延着方向前进0.10秒为目标坐标
                else
                    newTargetPos.Value = curPos;
                ...
                Debug.Log("XXXXXYYYYY==" + newTargetPos.Value.x + "/" + newTargetPos.Value.y + "/" + newTargetPos.Value.z);
                Debug.Log("XXXXXYYYYY==" + curPos.x + "/" + curPos.y + "/" + curPos.z);
                //EntityManager.SetComponentData<TargetPosition>(entities[0], newTargetPos);

                var findInfo = new FindWayInfo
                {
                    destination = newTargetPos.Value,
                    stoppingDistance = 0.1f,
                    onStop = null,
                };
                moveQuerys[0].StartFindWay(findInfo);//注意了，这个moveQuerys[0]设置的navagnet是那个中间节点，所有我需要在初始时候把他的位置设置成跟mainRole/MonsterRole一样
                //也就是要跟movequery的一样
            }
