...
public class SceneMgr : MonoBehaviour
{
	...
	public void Init(GameWorld world)
	{
        ...
        MovePlayManager.Instance.Init(world);
	}
    ...
    void LoadSceneInfo(int scene_id, Action<int> on_ok)
    {
        ...
        XLuaFramework.ResourceManager.GetInstance().LoadAsset<TextAsset>(SceneInfoPath+"scene_"+scene_id.ToString() +"/scene_info.json", delegate(UnityEngine.Object[] objs) {
            ...
            ResMgr.GetInstance().LoadSceneRes(scene_info.ResPathList, delegate(bool isOk)
            {
                ...
                for (int i = 0; i < scene_info.MonsterList.Count; i++)
                {
                        Debug.Log("scene_info.MonsterList" + scene_info.MonsterList[i].ToString());
                }
    ...
    public void LoadScene(int scene_id)
    {
        ...
        LoadSceneInfo(scene_id, delegate(int result){
            ...
            Debug.Log("LoadScene LoadSceneInfo delegate ");
            if (XLuaFramework.AppConfig.DebugMode)//false
            {
                ...
                Debug.Log("LoadScene baseWorldScenePath" + baseWorldScenePath);
            }
            else
            {
               ...
               XLuaFramework.ResourceManager.GetInstance().LoadNavMesh(baseWorldScenePath);//windows StreamingAssets/base_world_1001 乃场景的ab包
                Debug.Log("LoadScene baseWorldScenePath LoadNavMesh=" + baseWorldScenePath);
               // baseWorldScenePath = "Assets/AssetBundleRes/scene/base_world/base_world_" + scene_id + ".unity";
            }
    ...
    public Entity AddSceneObject(long uid, string content)
    {
        ...
        Debug.Log("mgr.type ="+ type+pos.x+pos.y+pos.z);
