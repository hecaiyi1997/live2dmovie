...
public class SceneHelper
{
    ...
    public static Vector3 GetSceneObjectPos(long uid)
    {
            Debug.Log("GetSceneObjectPos uid=" + uid);
            Entity entity=SceneMgr.Instance.GetSceneObject(uid);
            if (entity != Entity.Null)
            {
                Vector3 ret=EntityMgr.GetComponentObject<Transform>(entity).position;
                Debug.Log("GetSceneObjectPos=" + ret.x + ":" + ret.y + ":" + ret.z);
                return ret;
            }
