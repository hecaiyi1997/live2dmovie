...
namespace Cocos
{
    public class RatioBy : ActionInterval
    {
        protected float RatioDelta;
        protected float previousRatio;
        protected float startRatio;
        protected CoordinateStyle style;
        protected CinemachineVirtualCamera CineCam;

        public static RatioBy Create(float duration, float deltaRatio, CoordinateStyle style = CoordinateStyle.Local)
        {
            RatioBy ret = new RatioBy();
            if (ret != null && ret.InitWithStyle(duration, deltaRatio, style))
                return ret;
            return null;
        }

        public static RatioBy CreateLocal(float duration, float deltaRatio)
        {
            return Create(duration, deltaRatio, CoordinateStyle.Local);
        }

        public static RatioBy CreateAbs(float duration, float deltaRatio)
        {
            return Create(duration, deltaRatio, CoordinateStyle.Abs);
        }

        public static RatioBy CreateAnchored(float duration, float deltaRatio)
        {
            return Create(duration, deltaRatio, CoordinateStyle.Anchored);
        }

        private bool InitWithStyle(float duration, float deltaRatio, CoordinateStyle style)
        {
            bool ret = false;
            if (base.InitWithDuration(duration))
            {
                this.RatioDelta = deltaRatio;
                this.style = style;
                ret = true;
            }
            return ret;
        ...
        {
            Debug.Log("Ratio by clone");
            return RatioBy.Create(duration, RatioDelta, style);
        }
        ...
        public override void StartWithTarget(Transform target)
        {
            ...
            switch (style)
            {
                case CoordinateStyle.Local:
                    {
                        CineCam=target.GetComponent<CinemachineVirtualCamera>();
                        //CineCam.m_Lens.OrthographicSize = r;// 1.1f;
                        previousRatio = startRatio = CineCam.m_Lens.OrthographicSize;
                        break;
                    }
                case CoordinateStyle.Abs:
                    {
                        //previousRatio = startRatio = target.position;
                        break;
                    }
                case CoordinateStyle.Anchored:
                    {
                        //rectTrans = target as RectTransform;
                        //previousRatio = startRatio = rectTrans.anchoredRatio3D;
                        break;
                    }
                default:
        ...
        public override void Update(float t)
        {
            switch (style)
            {
                case CoordinateStyle.Local:
                    {
                        CineCam = target.GetComponent<CinemachineVirtualCamera>();//target就是拥有cine相机的那个GameObject，也就是他PlayAction
                        //CineCam.m_Lens.OrthographicSize = r;// 1.1f;
                        //previousRatio = startRatio = CineCam.m_Lens.OrthographicSize;

                        var currentRatio = CineCam.m_Lens.OrthographicSize;
                        var diff = currentRatio - previousRatio;
                        startRatio = startRatio + diff;
                        var newPos = startRatio + (RatioDelta * t);
                        CineCam.m_Lens.OrthographicSize = newPos;
                        previousRatio = newPos;
                        break;
                    }
                case CoordinateStyle.Abs:
                    {
